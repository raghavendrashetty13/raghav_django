...
def findcon(request):
    context_dict= {}
    if request.method=='POST':
        context_dict['Concessions']=con.objects.filter(Conc_Number__contains=request.POST['Conc'],)
    else:
        context_dict['Concessions']=con.objects.all
    return render (request,'concession/findcon.html',context=context_dict)
